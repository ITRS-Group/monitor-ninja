...
class ORM_Type_Password_Test extends PHPUnit_Framework_TestCase {
	...
	public function test_factory_from_array ($value, $expect) {
		...
		$from_array = TestClassA_Model::factory_from_array(array("password" => $value), array());
		$this->assertSame($expect, $from_array->get_password());
	}
	...
	public function test_factory_from_setiterator ($value, $expect) {
		$set = TestClassBPool_Model::all();
		$from_iterator = TestClassA_Model::factory_from_setiterator(array("password" => $value), false, array());
		$this->assertSame($expect, $from_iterator->get_password());
	}
	...
	public function test_using_setter ($value, $expect) {
		...
		$set_instance->set_password($value);
		$this->assertSame($expect, $set_instance->get_password());
	}
	...
	public function test_factory_from_array_set_not_existing () {
		...
		$this->assertSame("", $from_array->get_password());
	}
	...
	public function test_factory_from_setiterator_set_not_existing () {
		...
		$this->assertSame("", $from_iterator->get_password());
	}
	...
	public function test_factory_setter_set_not_existing () {
		...
		$this->assertSame("", $set_instance->get_password());
	}
	...
	public function invalid_data_provider () {
		return array(
			...
			array(array(), "'array' is not valid for password 'password'"),
			array((object)array(), "'object' is not valid for password 'password'"),
		);
	}
	...
	public function test_factory_from_array_invalid_values ($value, $expected) {
		...
		TestClassA_Model::factory_from_array(array("password" => $value), array());
	}
	...
	public function test_factory_from_setiterator_invalid_values ($value, $expected) {
		$this->setExpectedException('InvalidArgumentException', $expected);
		TestClassA_Model::factory_from_setiterator(array("password" => $value), false, array());
	}
