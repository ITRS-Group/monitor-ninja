...
class op5auth {
	...
	public function authorized_for( $authorization_point )
	{
		...
		if( $user->authorized_for( $authorization_point ) ) {
			//$this->log->log('debug', $user->username . ' is authorized for ' . $authorization_point);
			return true;
		}

		/* TODO: autorized_for_: fix short names better than this... */
		if( $user->authorized_for( 'authorized_for_' . $authorization_point ) ) {
			//$this->log->log('debug', $user . ' is authorized for ' . $authorization_point);
			return true;
		}
		//$this->log->log('debug', $user . ' is NOT authorized for ' . $authorization_point);
		return false;

	}

	/**
	 * Test if authorized for a certain object
	 * 
	 * @param $authorization_point string Name of authoirzation point
	 ...
	 */
	public function authorized_for_object( $authorization_point, $object_type, $object_definition, $case_sensitivity=true )
	{
		$user = $this->get_user();

		$access = $user->authorized_for($authorization_point);
	 ...
	 * 
	 * TODO: Deprecate? Use Livestatus lib directly? This is more a question to the database than auth?
	 * 
