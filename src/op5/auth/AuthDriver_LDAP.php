...
class op5AuthDriver_LDAP extends op5AuthDriver {
	...
	public function login($username, $password)
	{
		...
		if(isset($this->config['bind_with_upn']) && $this->config['bind_with_upn']) {
		...
		if($user_info === false) {
			$this->log->log('debug', 'No User info returned. (incorrect login/connection error)');
			return false;
		}
	...
	public function groups_available(array $grouplist)
	{
		...
		if(count($groups_group) > 0) {
			...
			foreach($groups_group as $group) {
				$filter .= '('.$this->config['groupkey'].'='.$this->ldap_escape($group).')';
			}
			$filter .= '))';

			foreach($groups_group as $group) {
				/* They are available for seaching, start with not available in LDAP, replace later */
				$result[ $group ] = false;
			}
		...
		if(count($groups_user) > 0) {
			...
			foreach($groups_user as $user) {
				...
				$filter .= '('.$this->config['userkey'].'='.$this->ldap_escape($user).')';
			}
			$filter .= '))';
	...
	private function resolve_groups_nonrecursive($object_dn)
	{
		$res = $this->ldap_query(array(
				$this->config['memberkey'] => $this->config['memberkey_is_dn'] ? $object_dn : $this->strip_dn($object_dn)
			),
			$this->config['group_base_dn'],
	...
	private function connect()
	{
		...
		if($enctype !== false &&
				...
				$enctype !== 'ssl')
		{
			$this->throw_error('Encryption is set, but is not "start_tls", "ssl" or "none"');
		}
	...
	private function bind($dn = false, $password = false)
	{
		if($dn === false) {
			...
			$result = @ldap_bind($this->conn); /* FIXME: Allow non-anonymous bind */
		} else {
			$this->log->log('debug', $this->config['name'] . ': Bindning as '.$dn.(($password===false)?', password=false':' with password set'));
	...
	private function bind_anon()
	{
		...
		if(!isset($this->config['bind_secret']) || $this->config['bind_secret'] === false) {
			/* Bind anonymously */
			if(!$this->bind()) {
