...
class Reports_Controller extends Base_reports_Controller
{
	...
	protected $reports_model = false; /**< An instance of the reports model we'll be using */
	...
	*/
	...
	/**
	 * What is this, and what's it good for?
	 */
	private function _print_state_breakdowns($source=false, $values=false, $type="hosts")
	{
		if (empty($values)) {
			return false;
		}
	...
	/**
	 * This *probably* only translates the unreadable strings returned from get_uptime into
	 * lowercase, prefix-free versions, but there could very well be something more sinister here.
	 *
	 * @param $data_arr (array) report source data, generated by status_reports_model:get_uptime()
	 * @param $sub_type (string) The report subtype. Can be 'host' or 'service'.
	 * @param $get_vars (string) query string containing values of options for the report<br>
	 * @param $start_time (int) Start timestamp for the report.
	 * @param $end_time (int) End timestamp for the report.
	 *
	 * @return (array) report info divided by states
	 */
	private function _get_multiple_state_info(&$data_arr, $sub_type, $get_vars, $start_time, $end_time, $type)
	{
		$prev_host = '';
		...
		if ($this->type == 'avail' && ($is_group || count($this->options[$this->options->get_value('report_type')]) > 1)) {
			...
			if (!empty($template_values) && count($template_values))
				for($i=0,$num_groups=count($template_values)  ; $i<$num_groups ; $i++) {
					$this->_reorder_by_host_and_service($template_values[$i]);
				}
	...
	*/
	...
	private function _print_states_for_services($host_name=false, $start_date=false, $end_date=false)
	{
