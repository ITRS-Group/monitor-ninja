...
$(document).ready(function() {
	...
	$('#new_schedule_report_form').submit(function(ev) {
		...
		$.ajax({
			...
			error: function(data) {
				$.notify(data.responseText, {type: "error"});
			},
			success: function(data) {
				...
				$.notify(_reports_schedule_create_ok, {type: "success"});
			},
			dataType: 'json'
		});
	});
	...
	$.ajax({
		...
		error: function(data) {
			if(data.responseText) {
				$.notify(_reports_schedule_send_error + ': ' + data.responseText, {type: "error"});
			} else {
				$.notify(_reports_schedule_send_error, {type: "error"});
			}
			img.attr('src', img_src);
		},
		dataType: 'json'
...
function validate_form(formData, jqForm, options) {
	...
	if (!saved_report_id) {
		$.notify(fatal_err_str, {type: "error", 'sticky': true});
		return false;
	}
