...
foreach ($multiple_states as $data) {
	...
	echo reports::format_multi_object_table($data, $groupname?:'Selected services', function($data) use ($options, $type, &$previous_hostname, $hide_host, &$i) {
		...
		if (!$hide_host && $data['states']['HOST_NAME'] != $previous_hostname) {
			...
			$res .= '<td colspan="6" class="multiple label"><strong>'. _('Services on host') .'</strong>: <a href="'.url::base(true).$type.'/generate?host_name[]='.$data['states']['HOST_NAME'].'&amp;'.$options->as_keyval_string(true).'">'.$name.'</a></td></tr><tr class="'.($i++%2?'even':'odd').'">';
			$previous_hostname = $data['states']['HOST_NAME'];
		}
		$name = $data['states']['SERVICE_DESCRIPTION'];
		return $res.'<td><a href="'.url::base(true).$type.'/generate?service_description[]='.$data['states']['HOST_NAME'].';'.$data['states']['SERVICE_DESCRIPTION'].'&amp;'.$options->as_keyval_string(true).'">'.$name.'</a></td>';
	}, true, false, $options['scheduleddowntimeasuptime'] == 2, $i);
	...
	}, true, true, $options['scheduleddowntimeasuptime'] == 2, $i);
}
