function lsfilter_list(config) {
  ...
  this.defaults = {
    ...
    request_url:
      _site_domain + _index_page + "/" + _controller_name + "/fetch_ajax",
    columns: false,
    toolbar: false,
    attach_head: false,
    notify: false,
  ...
  if (this.config.table) lsfilter_list_attach_events(this, this.config.table);
  if (this.config.totals) lsfilter_list_attach_events(this, this.config.totals);
  ...
  this.on = {
    update_ok: function(data) {
      ...
      if (!metadata) {
        var parser = new LSFilter(
          new LSFilterPreprocessor(),
      ...
      self.table_desc = new lsfilter_list_table_desc(
  ...
  this.set_sort = function(vis_column) {
    ...
    if (typeof lsfilter_main != "undefined")
      lsfilter_main.update(
        false,
        "list",
        this.sort_vis_column + (this.sort_ascending ? " asc" : " desc")
      );
  ...
  this.send_request = function(config) {
    if (
      typeof config.increment_items_in_view !== "undefined" &&
    ...
    this.active_ajax_request = $.ajax({
      ...
      success: function(data) {
        if (data == null) {
          self.handle_ajax_error_response({
            data: _("No valid response from server. Still logged in?")
          });
  ...
  this.update_float_header = function() {
    ...
    var header = $(thead).filter(function() {
      return !$(this).hasClass("floating-header");
    });
    var clone = $(thead).filter(function() {
      return $(this).hasClass("floating-header");
    });
  ...
  this.start_autorefresh_timer = function() {
    if (
      this.config.autorefresh_enabled === false ||
      isNaN(this.config.autorefresh_delay) ||
      this.config.autorefresh_delay < 1
    )
      return;
  ...
  this.handle_ajax_response = function(data) {
    ...
    if (data.data.length) {
      new_table = this.render_table(
        data,
        this.sort_vis_column,
        this.sort_ascending
      );
    } else {
      var empty_text;
      if (
        !(empty_text = $(this.config.table)
          .parents(".widget")
          .data("text-if-empty"))
      ) {
    ...
    if (this.config.toolbar) {
      this.config.toolbar.find(".toolbar-buttons").replaceContent(
        $("<ul/>").append(
          $.map(
            [$('<li id="filter_loading_status"/>')]
              .concat(listview_renderer_buttons[data.table] || [])
              .concat(listview_renderer_buttons.all || []),
            function(x) {
              if (typeof x == "function") {
                x = x();
              }
              return $('<li class="filter-query-button"/>')
                .html(x)
                .toArray();
            }
          )
  ...
  this.handle_ajax_error_response = function(data) {
    ...
    if (typeof data.data === "undefined") {
      alert.html(
        "Service Unavailable, you may attempt to refresh your browser"
      );
    } else {
  ...
  this.render_totals = function(table, totals) {
    ...
    if (totals) {
      for (var field in listview_renderer_totals) {
        if (field in totals) {
          var item = listview_renderer_totals[field](
            totals[field][1]
          ).wrapInner(link_query(totals[field][0]));
  ...
  this.render_table = function(data, sort_col, sort_asc) {
    ...
    if (data.length === 0) {
      return $(
        '<h2 class="lsfilter-noresult">' + _("Empty result set") + "</h2>"
      );
    }
  ...
  this.add_sort = function(element, vis_column, current) {
    ...
    element.append(
      $('<span class="lsfilter-sort-span" />').css(
        "background-image",
        "url(" + img + ")"
      )
    );
  ...
  this.banner_in_listview = function() {
    var currentLocation = window.location.ref.toString();
