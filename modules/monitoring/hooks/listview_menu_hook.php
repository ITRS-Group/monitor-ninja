...
Event::add ( 'ninja.menu.setup', function () {
	...
	if (isset ( $saved [$table] )) {
		foreach ( $saved [$table] as $object ) {
			...
			if ($count > $max_filters) {
				$menu->set ( 'Report.Notifications.All filters for ' . preg_replace ( '/\./', '&period;', $table ), listview::querylink ( sprintf ( '[saved_filters] filter_table = "%s"', $table ) ), $index, sprintf ( 'icon-16 x16-%s', 'filter' ) );
				break;
			}

			$menu->set ( 'Report.Notifications.' . $object->get_filter_name (), listview::querylink ( $object->get_filter () ), $index, sprintf ( 'icon-16 x16-%s', $icon ) );
		}
	}
} );
