...
class help
{
	...
	{
		$img_path = url::base(false).'application/views/icons/mui-question-mark-icon.svg';
		$controller = $controller !== null ? $controller : Router::$controller;
		return '<a class="help-icon" style="background: url(\''.$img_path.'\') no-repeat" data-popover="help:'.html::specialchars($controller).'.'.html::specialchars($key).'"></a>';
	}
}
