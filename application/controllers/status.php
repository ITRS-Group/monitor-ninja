		...
		if ($hostlist->count() > 0) {
			...
			foreach ($hostlist as $host) {
				...
				$service_states[$host->host_name][$host->service_state] = array(
				    ...
				    'status_link' => html::anchor('status/service/'.urlencode($host->host_name).'?hoststatustypes='.$hst_status_type.'&servicestatustypes='.$svc_status_type.'&style=detail', html::specialchars($host->state_count.' '.ucfirst(strtolower($this->current->status_text($host->service_state, 'service')))), array('class' => 'status-'.$this->current->status_text($host->service_state,'service'))),
