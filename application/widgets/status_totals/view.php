	...
	<div class="widget-content">
		<table class="max" style="border-spacing: 1px">
			...
			<?php foreach ($host_header as $row) { ?>
				...
				<td style="padding-right: 10px"><?php echo html::anchor($row['url'], html::specialchars($row['lable'].' '.$row['status'])) ?></td>
				<?php	} ?>
				<td class="status icon">
					<?php echo html::image('application/views/themes/default/images/icons/12x12/shield-not-warning.png',array('title' => $row['status'], 'alt' => $row['status'])); ?>
				</td>
			...
			<table style="border-spacing: 1px">
				<tr>
					<?php foreach ($service_header as $row) { ?>
							...
							<td style="padding-right: 10px"><?php echo html::anchor($row['url'], html::specialchars($row['lable'].' '.$row['status'])) ?></td>
					<?php } ?>
					<td class="status icon"><?php echo html::image('application/views/themes/default/images/icons/12x12/shield-not-warning.png',array('title' => $row['status'], 'alt' => $row['status'])); ?></td>
