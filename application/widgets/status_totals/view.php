...
<?php $i=0; foreach ($host_header as $row) { ?>
	...
	<tr>
		...
		<td><?php echo html::anchor('status/'.$target_method.'/'.$this->host.'?'.$grouptype_arg, html::specialchars($hosts['total_hosts'].' Hosts'), array('class' => 'status-total')) ?></td>
		<td class="status icon" style="padding: 4px 7px">
			<?php
				if ($hosts['hosts_problem'] > 0)
		...
		<td><?php echo html::anchor('status/host/'.$this->host.'/?hoststatustypes='.(nagstat::HOST_DOWN|nagstat::HOST_UNREACHABLE).'&'.$grouptype_arg, html::specialchars($hosts['hosts_problem'].' Problems'), array('class' => 'status-warning')) ?></td>
	</tr>
</table>
...
<table class="w-table service-totals">
	...
	<tr>
		<td class="status icon" style="padding: 4px 7px">
			<?php
				if ($services['services_problem'] > 0)
					echo html::image($this->add_path('icons/12x12/shield-warning.png'),array('title' => $label_all_problems, 'alt' => $label_all_problems, 'style' => 'margin-bottom: -2px'));
				else
					echo html::image($this->add_path('icons/12x12/shield-not-warning.png'),array('title' => $label_all_problems, 'alt' => $label_all_problems, 'style' => 'margin-bottom: -2px'));
			?>
		</td>
		<td><?php echo html::anchor('status/service/'.$this->host.'/?hoststatustypes='.$this->hoststatus.'&servicestatustypes='.(nagstat::SERVICE_UNKNOWN|nagstat::SERVICE_WARNING|nagstat::SERVICE_CRITICAL).'&'.$grouptype_arg, html::specialchars($services['services_problem'].' Problems'), array('class' => 'status-warning')) ?></td>
		<td colspan="2">&nbsp;</td>
	</tr>
</table>
