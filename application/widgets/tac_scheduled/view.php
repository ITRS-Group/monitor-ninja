...
<?php } ?>
		<table class="w-table">
			<?php for ($i = 0; $i < count($problem); $i++) { ?>
				<tr>
					<td class="dark"><?php echo html::image($this->add_path('icons/16x16/scheduled.png'), array('alt' => $problem[$i]['status'])) ?></td>
					<td>
			...
			<?php } if (count($problem) == 0) { ?>
				<tr>
					<td class="dark"><?php echo html::image($this->add_path('icons/16x16/scheduled-not.png'), array('alt' => $this->translate->_('N/A'))) ?></td>
