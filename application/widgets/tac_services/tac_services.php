...
class Tac_services_Widget extends widget_Core {
	...
	public function index($arguments=false, $master=false)
	{
		...
		if ($current_status->services_critical_scheduled) {
			$services_critical['status/service/all/0/'.nagstat::SERVICE_CRITICAL.'/'.nagstat::SERVICE_SCHEDULED_DOWNTIME] = $current_status->services_critical_scheduled.' '.$this->translate->_('Scheduled');
		}

		if ($current_status->services_critical_acknowledged) {
			$services_critical['status/service/all/0/'.nagstat::SERVICE_CRITICAL.'/'.nagstat::SERVICE_STATE_ACKNOWLEDGED] = $current_status->services_critical_acknowledged.' '.$this->translate->_('Acknowledged');
		}

		if ($current_status->services_critical_disabled) {
			$services_critical['status/service/all/0/'.nagstat::SERVICE_CRITICAL.'/'.nagstat::SERVICE_CHECKS_DISABLED ] = $current_status->services_critical_disabled.' '.$this->translate->_('Disabled');
		}
		...
		if ($current_status->services_warning_acknowledged) {
			$services_warning['status/service/all/0/'.nagstat::SERVICE_WARNING.'/'.nagstat::SERVICE_STATE_ACKNOWLEDGED] = $current_status->services_warning_acknowledged.' '.$this->translate->_('Acknowledged');
		}

		if ($current_status->services_warning_disabled) {
			$services_warning['status/service/all/0/'.nagstat::SERVICE_WARNING.'/'.nagstat::SERVICE_CHECKS_DISABLED ] = $current_status->services_warning_disabled.' '.$this->translate->_('Disabled');
		}
		...
		if ($current_status->services_unknown_unacknowledged) {
			$services_unknown['status/service/all/0/'.nagstat::SERVICE_UNKNOWN.'/'.(nagstat::HOST_UP|nagstat::HOST_PENDING).'/'.(nagstat::SERVICE_NO_SCHEDULED_DOWNTIME|nagstat::SERVICE_STATE_UNACKNOWLEDGED|nagstat::SERVICE_CHECKS_ENABLED)] =
		...
		if ($current_status->services_unknown_host_problem) {
			$services_unknown['status/service/all/0/'.nagstat::SERVICE_UNKNOWN.'/'.(nagstat::HOST_DOWN|nagstat::HOST_UNREACHABLE)] = $current_status->services_unknown_host_problem.' '.$this->translate->_('on Problem Hosts');
		}

		if ($current_status->services_unknown_scheduled) {
			$services_unknown['status/service/all/0/'.nagstat::SERVICE_UNKNOWN.'/'.nagstat::SERVICE_SCHEDULED_DOWNTIME] = $current_status->services_unknown_scheduled.' '.$this->translate->_('Scheduled');
		}

		if ($current_status->services_unknown_acknowledged) {
			$services_unknown['status/service/all/0/'.nagstat::SERVICE_UNKNOWN.'/'.nagstat::SERVICE_STATE_ACKNOWLEDGED] = $current_status->services_unknown_acknowledged.' '.$this->translate->_('Acknowledged');
		}

		if ($current_status->services_unknown_disabled) {
			$services_unknown['status/service/all/0/'.nagstat::SERVICE_UNKNOWN.'/'.nagstat::SERVICE_CHECKS_DISABLED ] = $current_status->services_unknown_disabled.' '.$this->translate->_('Disabled');
		}
		...
		if ($current_status->services_ok_disabled) {
			$services_ok_disabled['status/service/all/0/'.nagstat::SERVICE_OK.'/'.nagstat::SERVICE_CHECKS_DISABLED] = $current_status->services_ok_disabled.' '.$this->translate->_('Disabled');
		}

		# SERVICES PENDING DISABLED
		$services_pending_disabled = array();
		if ($current_status->services_pending_disabled) {
			$services_pending_disabled['status/service/all/0/'.nagstat::SERVICE_PENDING .'/'.nagstat::SERVICE_CHECKS_DISABLED] = $current_status->services_pending_disabled.' '.$this->translate->_('Disabled');
		}
