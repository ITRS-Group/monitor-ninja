...
class Tac_problems_Widget extends widget_Core {
	...
	public function index($arguments=false, $master=false)
	{
		...
		if ($current_status->hosts_down_unacknowledged) {
			...
			$problem[$i]['url'] = 'status/host/all/'.nagstat::HOST_DOWN.'/'.(nagstat::HOST_NO_SCHEDULED_DOWNTIME|nagstat::HOST_STATE_UNACKNOWLEDGED|nagstat::HOST_CHECKS_ENABLED);
		...
		if ($current_status->services_critical_unacknowledged) {
			...
			$problem[$i]['url'] = 'status/service/all/'.(nagstat::HOST_UP|nagstat::HOST_PENDING).'/'.nagstat::SERVICE_CRITICAL.'/'.(nagstat::SERVICE_NO_SCHEDULED_DOWNTIME|nagstat::SERVICE_STATE_UNACKNOWLEDGED|nagstat::SERVICE_CHECKS_ENABLED);
			...
			$problem[$i]['onhost'] = 'status/service/all/'.(nagstat::HOST_DOWN|nagstat::HOST_UNREACHABLE).'/'.nagstat::SERVICE_CRITICAL;
		...
		if ($current_status->hosts_unreachable_unacknowledged) {
			...
			$problem[$i]['url'] = 'status/host/all/'.nagstat::HOST_UNREACHABLE.'/'.(nagstat::HOST_NO_SCHEDULED_DOWNTIME|nagstat::HOST_STATE_UNACKNOWLEDGED|nagstat::HOST_CHECKS_ENABLED);
		...
		if ($current_status->services_warning_unacknowledged) {
			...
			$problem[$i]['url'] = 'status/service/all/'.(nagstat::HOST_UP|nagstat::HOST_PENDING).'/'.nagstat::SERVICE_WARNING.'/'.(nagstat::SERVICE_NO_SCHEDULED_DOWNTIME|nagstat::SERVICE_STATE_UNACKNOWLEDGED|nagstat::SERVICE_CHECKS_ENABLED);
			...
			$problem[$i]['onhost'] = 'status/service/all/'.(nagstat::HOST_DOWN|nagstat::HOST_UNREACHABLE).'/'.nagstat::SERVICE_WARNING;
		...
		if ($current_status->services_unknown_unacknowledged) {
			...
			$problem[$i]['url'] = 'status/service/all/0/'.nagstat::SERVICE_UNKNOWN.'/'.(nagstat::HOST_UP|nagstat::HOST_PENDING).'/'.(nagstat::SERVICE_NO_SCHEDULED_DOWNTIME|nagstat::SERVICE_STATE_UNACKNOWLEDGED|nagstat::SERVICE_CHECKS_ENABLED);
			...
			$problem[$i]['onhost'] = 'status/service/all/0/'.nagstat::SERVICE_UNKNOWN.'/'.(nagstat::HOST_DOWN|nagstat::HOST_UNREACHABLE);
