...
class Tac_problems_Widget extends widget_Base {
	...
	public function index()
	{
		...
		if ($current_status->hst->outages) {
			$problem[$i]['type'] = _('Network');
			$problem[$i]['status'] = _('Outages');
			$problem[$i]['url'] = 'outages/index/';
			$problem[$i]['title'] = $current_status->hst->outages.' '._('Network outages');
			$problem[$i]['no'] = 0;
		...
		if ($current_status->hst->down_and_unhandled) {
			...
			$problem[$i]['title'] = $current_status->hst->down_and_unhandled.' '._('Unhandled problems');
			$problem[$i]['no'] = 0;
		...
		if ($current_status->svc->critical_and_unhandled) {
			...
			$problem[$i]['title'] = $current_status->svc->critical_and_unhandled.' '._('Unhandled problems');
			$problem[$i]['no'] = $current_status->svc->critical_on_down_host;
			...
			$problem[$i]['title2'] = $current_status->svc->critical_on_down_host.' '._('on problem hosts');
		...
		if ($current_status->hst->unreachable_and_unhandled) {
			...
			$problem[$i]['title'] = $current_status->hst->unreachable_and_unhandled.' '._('Unhandled problems');
		...
		if ($current_status->svc->warning_and_unhandled) {
			...
			$problem[$i]['title'] = $current_status->svc->warning_and_unhandled.' '._('Unhandled problems');
			$problem[$i]['no'] = $current_status->svc->warning_on_down_host;
			...
			$problem[$i]['title2'] = $current_status->svc->warning_on_down_host.' '._('on problem hosts');
		...
		if ($current_status->svc->unknown_and_unhandled) {
			...
			$problem[$i]['title'] = $current_status->svc->unknown_and_unhandled.' '._('Unhandled problems');
			$problem[$i]['no'] = $current_status->svc->unknown_on_down_host;
			...
			$problem[$i]['title2'] = $current_status->svc->unknown_on_down_host.' '._('on problem hosts');
