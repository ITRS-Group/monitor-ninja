...
class Current_status_Model extends Model
{
	...
	/**
	*	Fecth servie total summary information
	* 	for a specific servicegroup (or all)
	*/
	public function get_servicegroup_service_totals_summary($servicegroup=false, $servicestatus=false)
	{
			$servicegroup = trim($servicegroup);
			if (empty($servicegroup)) {
				return false;
			...
			$servicelist_str = implode(',', $servicelist);
		...
		WHERE
			...
			ssg.servicegroup = sg.id AND
			s.id=ssg.service AND
			s.id IN (".$servicelist_str.") ".$filter_sql."
		GROUP BY
			s.current_state,
			sg.id
		ORDER BY
