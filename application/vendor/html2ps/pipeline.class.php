...
if(empty($HTML2PS_CACHE_DIR))
	die('Variable $HTML2PS_CACHE_DIR must be set by caller to point to writable cache directory');

if(empty($HTML2PS_WRITER_TEMPDIR))
	die('Variable $HTML2PS_WRITER_TEMPDIR must be set by caller to point to writable cache directory');

if(empty($HTML2PS_OUTPUT_FILE_DIRECTORY))
	die('Variable $HTML2PS_OUTPUT_FILE_DIRECTORY must be set by caller to point to writable cache directory');
...
#die(nl2br(print_r($box,true)));
    ...
    $this->_dispatcher->fire('before-document', array('pipeline' => &$this, 
                                                      ...
                                                      'media' => &$media));
# die("sidor = ".print_r($page_heights,true));
