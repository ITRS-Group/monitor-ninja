...
/*
 080523 FREF
	Changed CACHE_DIR define to variable $HTML2PS_CACHE_DIR to be reconfigurable
	Changed OUTPUT_FILE_DIRECTORY to variable $HTML2PS_OUTPUT_FILE_DIRECTORY to be reconfigurable
	Changed WRITER_TEMPDIR to variable $HTML2PS_WRITER_TEMPDIR to be reconfigurable (no trailing slash)

	The above changes are necessary, since the op5common package is to be general purpose, allowing different
	users to create pdf files without having a chmod 777 directory under this dir.
	Cannot use /tmp since data might be sensitive.

 NOTES:
	HTML2PS_DIR/cache still is used for reading fonts. When writing encodings etc, $HTML2PS_CACHE_DIR is used.
	Directories _should_ end with a trailing '/' EXCEPT for $HTML2PS_WRITER_TEMPDIR!
*/
...
define('OUTPUT_DEFAULT_NAME','report.pdf');
