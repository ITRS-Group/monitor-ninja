...
/*
function start($node, $depth) 
{ 
	print str_repeat("&nbsp;", $depth). '&lt;'.$node->tagname();
	if($node->tagname() == 'input')
		print "&nbsp; name='".$node->get_attribute('name')."'";
	if($node->tagname() == 'img')
		print "&nbsp; src='".$node->get_attribute('src')."'";
	print '&gt;';  
}
function stop($node, $depth)  { if($node->tagname()=='input') return; print str_repeat("&nbsp;", $depth). '&lt;/'.$node->tagname().'&gt;<br>'; }
*/
