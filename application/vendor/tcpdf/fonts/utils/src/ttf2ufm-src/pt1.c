	...
	doagain:
		...
		if(ISDBG(QUAD)) {
			fprintf(stderr, "%s: trying 0x%x (%g %g) (%g %g) (%g %g) (%g %g)\n  ", g->name,
				(unsigned int)ge,  ge->prev->fx3, ge->prev->fy3, ge->fx1, ge->fy1, ge->fx2, ge->fy2,
				ge->fx3, ge->fy3);
		}
		for(i=0; i<2; i++) { /* first for x then for y */
			...
			if(ISDBG(QUAD))
				fprintf(stderr, "%s: 0x%x: %d pts(%c): ", 
					g->name, (unsigned int)ge, np-oldnp, i? 'y':'x');
...
#define TIMESLARGER 10.	/* how much larger must be a curve to not change too much */
	...
	for (ge = g->entries; ge != 0; ge = next) {
		...
		if(ISDBG(FCONCISE))  {
			...
			g->name, (unsigned int)pge, (unsigned int)ge, (unsigned int)nge);
			dumppaths(g, pge, nge);
		}
...
#define MAXDOTS	((NREFDOTS+1)*12)
	...
	restart:
		...
		if(ISDBG(FCONCISE))
			fprintf(stderr, ": 0x%x\nnext -> 0x%p ", (unsigned int)pge, nge);
		...
		while(pge!=nge) {
			...
			if(ISDBG(FCONCISE))
				fprintf(stderr, "(p=%p/%x n=0x%x/%x) ", pge, pgex->flags & GEXF_JLINE, 
					(unsigned int)nge, ngex->flags & GEXF_JLINE);
...
)
{
	...
	if(transform)
	  fprintf(afm_file, "C %d ; WX %d ; N %s ; G %d ; B %d %d %d %d ;\n",
...
)
{
  ...
  // Modified to output character specific bounding boxes as in AFM files
	if(transform)
...
)
{
	...
	for (ge = from; ge != 0 && ge != to; ge = ge->next) {
		if(ge->type == GE_LINE || ge->type == GE_CURVE) {
			if (ISDBG(REVERSAL))
				fprintf(stderr, "reverse path 0x%x <- 0x%x, 0x%x\n", (unsigned int)ge, (unsigned int)ge->prev, (unsigned int)ge->bkwd);
