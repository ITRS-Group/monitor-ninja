...
populate_map:
    ...
    orig_code = code;
		...
		if(code == 0)
			continue; /* .notdef */
		encoding[i] = code;
    if (glyph_list[code].orig_code == -1)
    {
      glyph_list[code].orig_code = orig_code;
    }
	}
  ...
  while (gindex != 0)
  {
    if (glyph_list[gindex].orig_code == -1)
    {
      glyph_list[gindex].orig_code = charcode;
    }
    else if (glyph_list[gindex].orig_code != charcode)
    {
		  fprintf(stderr, "Warning: Glyph %ud = CharCode %d != %ud\n", gindex, glyph_list[gindex].orig_code, charcode);
    }
