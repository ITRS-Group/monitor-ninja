		...
		<td class="icon" style="text-align: left">
			<?php
				...
				if (Kohana::config('config.pnp4nagios_path')!==false)
					echo (pnp::has_graph($host->host_name))  ? '<a href="' . Kohana::config('config.site_domain') . '/index.php/pnp/?host='.urlencode($host->host_name).'" style="border: 0px">'.html::image($this->add_path('icons/16x16/pnp.png'), array('alt' => 'Show performance graph', 'title' => 'Show performance graph')).'</a> &nbsp;' : '';
			...
			<?php
				...
				if (Kohana::config('config.pnp4nagios_path')!==false) {
					if (pnp::has_graph($service->host_name, urlencode($service->service_description)))
						echo '<a href="' . Kohana::config('config.site_domain') . '/index.php/pnp/?host='.urlencode($service->host_name).'&srv='.urlencode($service->service_description).'" style="border: 0px">'.html::image($this->add_path('icons/16x16/pnp.png'), array('alt' => 'Show performance graph', 'title' => 'Show performance graph')).'</a> &nbsp;';
				}
