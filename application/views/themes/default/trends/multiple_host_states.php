...
<?php foreach ($multiple_states as $data) { ?>
		<table summary="<?php echo _('Host state breakdown') ?>" id="multiple_hosts" border="1">
			...
			<?php if (!$options['use_alias']) {
				...
				$host_link = str_replace('avail/generate', 'trends/generate?1', $host_link);?>
			...
			<tr class="<?php echo ($i%2 == 0) ? 'even' : 'odd'?>">
				...
				<td class="data"><?php echo $data['group_unreachable'] ?> % <?php echo html::image($this->add_path('icons/12x12/shield-'.($data['group_unreachable'] > 0 ? '' : 'not-').'unreachable.png'),
							array( 'alt' => _('Unreachable'), 'title' => _('Unreachable'), 'style' => 'height: 12px; width: 11px')) ?></td>
				<td class="data"><?php echo $data['group_down'] ?> % <?php echo html::image($this->add_path('icons/12x12/shield-'.($data['group_down'] > 0 ? '' : 'not-').'down.png'),
							array( 'alt' => _('Down'), 'title' => _('Down'), 'style' => 'height: 12px; width: 11px')) ?></td>
