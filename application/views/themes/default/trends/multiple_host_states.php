...
<br />
<?php
	echo html::anchor(Kohana::config('reports.reports_link').'/generate?type=avail&report_type='.$report_type.$selected_objects.$get_vars, html::image($this->add_path('icons/16x16/availability.png'), array('title' => _('View corresponding Availability report'))), array('style' => 'border: 0px; margin-right: 5px; margin-bottom: -4px; display: block; float: left'));
	echo html::anchor(Kohana::config('reports.reports_link').'/generate?type=avail&report_type='.$report_type.$selected_objects.$get_vars, _('View corresponding Availability report'));
?>
...
<?php foreach ($multiple_states as $data) { ?>
		<table summary="<?php echo _('Host state breakdown') ?>" id="multiple_hosts" border="1">
			<tr>
				...
				<th class="headerNone"><?php echo _('Down') ?></th>
			...
			<?php if (!$use_alias) {
				...
				<td class="data">
					...
					<?php echo html::anchor(Kohana::config('reports.reports_link').'/generate?type=avail&host_name[]='.$data['HOST_NAME'][$i].$get_vars, html::image($this->add_path('icons/16x16/availability.png'), array('title' => _('Availability report for this Host'))), array('style' => 'border: 0px')) ?>
				...
				<td class="data">
					...
					<?php echo html::image($this->add_path('icons/12x12/shield-'.(reports::format_report_value($data['up'][$i]) > 0 ? '' : 'not-').'up.png'),
											array( 'alt' => _('Up'), 'title' => _('Up'), 'style' => 'height: 12px; width: 11px')); ?>
				</td>
				<td class="data">
					...
					<?php echo html::image($this->add_path('icons/12x12/shield-'.(reports::format_report_value($data['unreachable'][$i]) > 0 ? '' : 'not-').'unreachable.png'),
										array( 'alt' => _('Unreachable'), 'title' => _('Unreachable'), 'style' => 'height: 12px; width: 11px')) ?></td>
				<td class="data"><?php echo reports::format_report_value($data['down'][$i]) ?> % <?php echo html::image($this->add_path('icons/12x12/shield-'.(reports::format_report_value($data['down'][$i]) > 0 ? '' : 'not-').'down.png'),
							array( 'alt' => _('Down'), 'title' => _('Down'), 'style' => 'height: 12px; width: 11px')) ?></td>
			...
			<tr class="group-average <?php echo ($i%2 == 0) ? 'even' : 'odd'?>">
				<td colspan="2"><?php echo ($use_average==0) ? _('Group availability (SLA)') : _('Average'); ?></td>
