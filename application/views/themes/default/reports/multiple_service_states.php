...
<div class="state_services">
	<?php } ?>
		<table summary="<?php echo _('State breakdown for host services') ?>" class="multiple_services" style="margin-top: 15px" border="1" <?php echo ($create_pdf) ? 'cellpadding="5"' : ''; ?>>
			<tr>
				...
				<th <?php echo ($create_pdf) ? 'style="font-weight:bold; width: 90px;text-align: right; background-color: #e2e2e2; font-size: 0.9em"' : 'class="headerNone" style="width: 80px"';?>><?php echo _('OK') ?></th>
				<th <?php echo ($create_pdf) ? 'style="font-weight:bold; width: 90px;text-align: right; background-color: #e2e2e2; font-size: 0.9em"' : 'class="headerNone" style="width: 80px"';?>><?php echo _('Warning') ?></th>
				<th <?php echo ($create_pdf) ? 'style="font-weight:bold; width: 90px;text-align: right; background-color: #e2e2e2; font-size: 0.9em"' : 'class="headerNone" style="width: 80px"';?>><?php echo _('Unknown') ?></th>
				<th <?php echo ($create_pdf) ? 'style="font-weight:bold; width: 90px;text-align: right; background-color: #e2e2e2; font-size: 0.9em"' : 'class="headerNone" style="width: 80px"';?>><?php echo _('Critical') ?></th>
				<th <?php echo ($create_pdf) ? 'style="font-weight:bold; width: 105px;text-align: right; background-color: #e2e2e2; font-size: 0.9em"' : 'class="headerNone" style="width: 80px"';?>><?php echo _('Undetermined') ?></th>
				</tr>
		<?php } ?>
			...
			<?php if (!$use_alias && $sg_no == 0) { ?>
				<td colspan="6" class="multiple label"><strong><?php echo _('Services on host') ?></strong>: <?php echo $create_pdf != false ? html::anchor(base_url::get().str_replace('/ninja/index.php/', null, $data['host_link'][$i]), $data['HOST_NAME'][$i]) :'<a href="'.str_replace('&','&amp;',$data['host_link'][$i]).'">' . $data['HOST_NAME'][$i] . '</a>'; ?></td>
			<?php } elseif ($sg_no == 0) { ?>
				<td colspan="6" class="multiple label"><strong><?php echo _('Services on host') ?></strong>: <?php echo $this->_get_host_alias($data['HOST_NAME'][$i]) ?> (<?php echo $create_pdf != false ? html::anchor(base_url::get().str_replace('/ninja/index.php/', null, $data['host_link'][$i]), $data['HOST_NAME'][$i]) : '<a href="'.str_replace('&','&amp;',$data['host_link'][$i]).'">' . $data['HOST_NAME'][$i] . '</a>'; ?>)</td>
			<?php } ?>
			</tr>
			...
			<tr class="<?php echo ($i%2==0 ? 'even' : 'odd') ?>">
				<td <?php echo ($create_pdf) ? 'style="width: 90px; font-size: 0.9em; background-color: '.$bg_color.'"' : ''; ?>><?php echo _('Average') ?></td>
			...
			<?php if ($sg_no == 0) { ?>
			<tr class="<?php echo ($i%2==0 ? 'even' : 'odd') ?>">
				<td <?php echo ($create_pdf) ? 'style="width: 222px; font-size: 0.9em; background-color: '.$bg_color.'"' : ''; ?>><?php if ($use_average==0) { ?><?php echo _('Group availability (SLA)') ?> <?php } else { ?><?php echo _('Average') ?><?php } ?></td>
...
<div class="state_services">
	<table summary="<?php echo _('State breakdown for host services') ?>" <?php echo ($create_pdf) ? 'style="border: 1px solid #cdcdcd" cellpadding="5"' : 'class="multiple_services" style="margin-bottom: 15px"';?>>
		<tr>
			<th <?php echo ($create_pdf) ? 'style="width: 222px; font-weight: bold; background-color: #e2e2e2; font-size: 0.9em"' : 'class="headerNone left" ';?>><?php echo ((!$create_pdf) ? help::render('average_and_sla') : '').' '._('Average and Group availability for all selected services') ?></th>
			<th <?php echo ($create_pdf) ? 'style="width: 90px; font-weight: bold; text-align: right; background-color: #e2e2e2; font-size: 0.9em"' : 'class="headerNone" style="width: 80px"';?>><?php echo _('OK') ?></th>
			<th <?php echo ($create_pdf) ? 'style="width: 90px; font-weight: bold; text-align: right; background-color: #e2e2e2; font-size: 0.9em"' : 'class="headerNone" style="width: 80px"';?>><?php echo _('Warning') ?></th>
			<th <?php echo ($create_pdf) ? 'style="width: 90px; font-weight: bold; text-align: right; background-color: #e2e2e2; font-size: 0.9em"' : 'class="headerNone" style="width: 80px"';?>><?php echo _('Unknown') ?></th>
			<th <?php echo ($create_pdf) ? 'style="width: 90px; font-weight: bold; text-align: right; background-color: #e2e2e2; font-size: 0.9em"' : 'class="headerNone" style="width: 80px"';?>><?php echo _('Critical') ?></th>
			<th <?php echo ($create_pdf) ? 'style="width: 105px; font-weight: bold; text-align: right; background-color: #e2e2e2; font-size: 0.9em"' : 'class="headerNone" style="width: 80px"';?>><?php echo _('Undetermined') ?></th>
		</tr>
		<?php if ($use_average==0) {  ?>
		<tr class="even">
			<td <?php echo ($create_pdf) ? 'style="width: 222px; font-size: 0.9em; background-color: #ffffff"' : ''; ?>><?php echo _('Average');?></td>
