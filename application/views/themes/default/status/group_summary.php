		...
		<tbody>
			...
			<tr class="<?php echo ($i%2 == 0) ? 'odd' : 'even'?>">
				<td class="bl" style="white-space: normal">
					<?php echo html::anchor('status/'.$grouptype.'group/?group='.urlencode($details->groupname).'&style=overview', htmlspecialchars($details->alias)) ?><br />
					(<?php echo html::anchor('extinfo/details/?type='.$grouptype.'group&host='.urlencode($details->groupname), $details->groupname) ?>)
				</td>
				<td class="icon">
					<?php if (nacoma::link()===true)
						echo nacoma::link('configuration/configure/?type='.$grouptype.'group&name='.urlencode($details->groupname), 'icons/16x16/nacoma.png', sprintf($this->translate->_('Configure this %sgroup'), $grouptype));
					?>
				</td>
					...
					<td style="line-height: 20px; white-space: normal">
						<?php
							if ($details->services_ok > 0) {
								...
								echo html::anchor('status/service/?name='.urlencode($details->groupname).'&servicestatustypes='.nagstat::SERVICE_OK.'&hoststatustypes='.$hoststatustypes.'&serviceprops='.$serviceproperties.'&hostproperties='.$hostproperties.'&style=detail&group_type='.$grouptype.'group', $details->services_ok.' '.$label_ok, array('class' => 'status-ok')).'<br />';
							}
							...
							if ($details->services_warning > 0) {
								...
								echo html::anchor('status/service/?name='.urlencode($details->groupname).'&servicestatustypes='.nagstat::SERVICE_WARNING.'&hoststatustypes='.$hoststatustypes.'&serviceprops='.$serviceproperties.'&hostproperties='.$hostproperties.'&group_type='.$grouptype.'group', $details->services_warning.' '.$label_warning, array('class' => 'status-warning')).': ';

								$c = 0;
								if ($details->services_warning_unhandled > 0) {
									echo ($c != 0 ? ', ' : '').html::anchor('status/service/?name='.urlencode($details->groupname).'&style=detail&servicestatustypes='.nagstat::SERVICE_WARNING.'&hoststatustypes='.(nagstat::HOST_UP|nagstat::HOST_PENDING).'&serviceprops='.(nagstat::SERVICE_NO_SCHEDULED_DOWNTIME|nagstat::SERVICE_STATE_UNACKNOWLEDGED|nagstat::SERVICE_CHECKS_ENABLED).'&group_type='.$grouptype.'group', $details->services_warning_unhandled.' '.$label_unhandled);
									$c++;
								}
								if ($details->services_warning_host_problem > 0) {
									echo ($c != 0 ? ', ' : '').html::anchor('status/service/?name='.urlencode($details->groupname).'&style=detail&servicestatustypes='.nagstat::SERVICE_WARNING.'&hoststatustypes='.(nagstat::HOST_DOWN|nagstat::HOST_UNREACHABLE).'&group_type='.$grouptype.'group', $details->services_warning_host_problem.' '.$label_on_problem_hosts);
									$c++;
								}
								if ($details->services_warning_scheduled > 0) {
									echo ($c != 0 ? ', ' : '').html::anchor('status/service/?name='.urlencode($details->groupname).'&style=detail&servicestatustypes='.nagstat::SERVICE_WARNING.'&serviceprops='.nagstat::SERVICE_SCHEDULED_DOWNTIME.'&group_type='.$grouptype.'group', $details->services_warning_scheduled.' '.$label_scheduled);
									$c++;
								}
								if ($details->services_warning_acknowledged > 0) {
									echo ($c != 0 ? ', ' : '').html::anchor('status/service/?name='.urlencode($details->groupname).'&style=detail&servicestatustypes='.nagstat::SERVICE_WARNING.'&serviceprops='.nagstat::SERVICE_STATE_ACKNOWLEDGED.'&group_type='.$grouptype.'group', $details->services_warning_acknowledged.' '.$label_acknowledged);
									$c++;
								}
								if ($details->services_warning_disabled > 0) {
									echo ($c != 0 ? ', ' : '').html::anchor('status/service/?name='.urlencode($details->groupname).'&style=detail&servicestatustypes='.nagstat::SERVICE_WARNING.'&serviceprops='.nagstat::SERVICE_CHECKS_DISABLED.'&group_type='.$grouptype.'group', $details->services_warning_disabled.' '.$label_disabled);
									$c++;
								}
								echo '<br />';
							}
							...
							if ($details->services_unknown > 0) {
								...
								echo html::anchor('status/service/?name='.urlencode($details->groupname).'&servicestatustypes='.nagstat::SERVICE_UNKNOWN.'&hoststatustypes='.$hoststatustypes.'&serviceprops='.$serviceproperties.'&hostproperties='.$hostproperties.'&group_type='.$grouptype.'group', $details->services_unknown.' '.$label_unknown, array('class' => 'status-unknown')).': ';

								$c = 0;
								if ($details->services_unknown_unhandled > 0) {
									echo ($c != 0 ? ', ' : '').html::anchor('status/service/?name='.urlencode($details->groupname).'&style=detail&servicestatustypes='.nagstat::SERVICE_UNKNOWN.'&hoststatustypes='.(nagstat::HOST_UP|nagstat::HOST_PENDING).'&serviceprops='.(nagstat::SERVICE_NO_SCHEDULED_DOWNTIME|nagstat::SERVICE_STATE_UNACKNOWLEDGED|nagstat::SERVICE_CHECKS_ENABLED).'&group_type='.$grouptype.'group', $details->services_unknown_unhandled.' '.$label_unhandled);
									$c++;
								}
								if ($details->services_unknown_host_problem > 0) {
									echo ($c != 0 ? ', ' : '').html::anchor('status/service/?name='.urlencode($details->groupname).'&style=detail&servicestatustypes='.nagstat::SERVICE_UNKNOWN.'&hoststatustypes='.(nagstat::HOST_DOWN|nagstat::HOST_UNREACHABLE).'&group_type='.$grouptype.'group', $details->services_unknown_host_problem.' '.$label_on_problem_hosts);
									$c++;
								}
								if ($details->services_unknown_scheduled > 0) {
									echo ($c != 0 ? ', ' : '').html::anchor('status/service/?name='.urlencode($details->groupname).'&style=detail&servicestatustypes='.nagstat::SERVICE_UNKNOWN.'&serviceprops='.nagstat::SERVICE_SCHEDULED_DOWNTIME.'&group_type='.$grouptype.'group', $details->services_unknown_scheduled.' '.$label_scheduled);
									$c++;
								}
								if ($details->services_unknown_acknowledged > 0) {
									echo ($c != 0 ? ', ' : '').html::anchor('status/service/?name='.urlencode($details->groupname).'&style=detail&servicestatustypes='.nagstat::SERVICE_UNKNOWN.'&serviceprops='.nagstat::SERVICE_STATE_ACKNOWLEDGED.'&group_type='.$grouptype.'group', $details->services_unknown_acknowledged.' '.$label_acknowledged);
									$c++;
								}
								if ($details->services_unknown_disabled > 0) {
									echo ($c != 0 ? ', ' : '').html::anchor('status/service/?name='.urlencode($details->groupname).'&style=detail&servicestatustypes='.nagstat::SERVICE_UNKNOWN.'&serviceprops='.nagstat::SERVICE_CHECKS_DISABLED.'&group_type='.$grouptype.'group', $details->services_unknown_disabled.' '.$label_disabled);
									$c++;
								}
								echo '<br />';
							}
							...
							if ($details->services_critical > 0) {
								...
								echo html::anchor('status/service/?name='.urlencode($details->groupname).'&servicestatustypes='.nagstat::SERVICE_CRITICAL.'&hoststatustypes='.$hoststatustypes.'&serviceprops='.$serviceproperties.'&hostproperties='.$hostproperties.'&group_type='.$grouptype.'group', $details->services_critical.' '.$label_critical, array('class' => 'status-critical')).': ';

								$c = 0;
								if ($details->services_critical_unhandled > 0) {
									echo ($c != 0 ? ', ' : '').html::anchor('status/service/?name='.urlencode($details->groupname).'&style=detail&servicestatustypes='.nagstat::SERVICE_CRITICAL.'&hoststatustypes='.(nagstat::HOST_UP|nagstat::HOST_PENDING).'&serviceprops='.(nagstat::SERVICE_NO_SCHEDULED_DOWNTIME|nagstat::SERVICE_STATE_UNACKNOWLEDGED|nagstat::SERVICE_CHECKS_ENABLED).'&group_type='.$grouptype.'group', $details->services_critical_unhandled.' '.$label_unhandled);
									$c++;
								}
								if ($details->services_critical_host_problem > 0) {
									echo ($c != 0 ? ', ' : '').html::anchor('status/service/?name='.urlencode($details->groupname).'&style=detail&servicestatustypes='.nagstat::SERVICE_CRITICAL.'&hoststatustypes='.(nagstat::HOST_DOWN|nagstat::HOST_UNREACHABLE).'&group_type='.$grouptype.'group', $details->services_critical_host_problem.' '.$label_on_problem_hosts);
									$c++;
								}
								if ($details->services_critical_scheduled > 0) {
									echo ($c != 0 ? ', ' : '').html::anchor('status/service/?name='.urlencode($details->groupname).'&style=detail&servicestatustypes='.nagstat::SERVICE_CRITICAL.'&serviceprops='.nagstat::SERVICE_SCHEDULED_DOWNTIME.'&group_type='.$grouptype.'group', $details->services_critical_scheduled.' '.$label_scheduled);
									$c++;
								}
								if ($details->services_critical_acknowledged > 0) {
									echo ($c != 0 ? ', ' : '').html::anchor('status/service/?name='.urlencode($details->groupname).'&style=detail&servicestatustypes='.nagstat::SERVICE_CRITICAL.'&serviceprops='.nagstat::SERVICE_STATE_ACKNOWLEDGED.'&group_type='.$grouptype.'group', $details->services_critical_acknowledged.' '.$label_acknowledged);
									$c++;
								}
								if ($details->services_critical_disabled > 0) {
									echo ($c != 0 ? ', ' : '').html::anchor('status/service/?name='.urlencode($details->groupname).'&style=detail&servicestatustypes='.nagstat::SERVICE_CRITICAL.'&serviceprops='.nagstat::SERVICE_CHECKS_DISABLED.'&group_type='.$grouptype.'group', $details->services_critical_disabled.' '.$label_disabled);
									$c++;
								}
								echo '<br />';
							}
							...
							if ($details->services_pending > 0) {
								echo html::image($this->add_path('icons/12x12/shield-pending.png'), array('alt' => $label_pending, 'title' => $label_pending, 'class' => 'status-default'));
								echo html::anchor('status/service/?name='.urlencode($details->groupname).'&style=detail&servicestatustypes='.nagstat::SERVICE_PENDING.'&hoststatustypes='.$hoststatustypes.'&serviceproperties='.$serviceproperties.'&hostproperties='.$hostproperties.'&group_type='.$grouptype.'group', $details->services_pending.' '.$label_pending, array('class' => 'status-pending'));
							}
