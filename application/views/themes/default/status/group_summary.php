					...
					<td style="line-height: 20px">
						<?php
							if ($details->services_ok > 0) {
								...
								echo html::anchor('status/'.$grouptype.'group/'.urlencode($details->groupname).'?servicestatustypes='.nagstat::SERVICE_OK.'&hoststatustypes='.$hoststatustypes.'&serviceprops='.$serviceproperties.'&hostproperties='.$hostproperties.'&style=detail', $details->services_ok.' '.$label_ok).'<br />';
							}
							...
							if ($details->services_warning > 0) {
								...
								echo html::anchor('status/'.$grouptype.'group/'.urlencode($details->groupname).'?servicestatustypes='.nagstat::SERVICE_WARNING.'&hoststatustypes='.$hoststatustypes.'&serviceprops='.$serviceproperties.'&hostproperties='.$hostproperties.'&style=detail', $details->services_warning.' '.$label_warning).': ';
								$c = 0;
								if ($details->services_warning_unhandled > 0) {
									echo ($c != 0 ? ', ' : '').html::anchor('status/'.$grouptype.'group/'.urlencode($details->groupname).'?style=detail&servicestatustypes='.nagstat::SERVICE_WARNING.'&hoststatustypes='.(nagstat::HOST_UP|nagstat::HOST_PENDING).'&serviceprops='.(nagstat::SERVICE_NO_SCHEDULED_DOWNTIME|nagstat::SERVICE_STATE_UNACKNOWLEDGED|nagstat::SERVICE_CHECKS_ENABLED), $details->services_warning_unhandled.' '.$label_unhandled);
									$c++;
								}
								if ($details->services_warning_host_problem > 0) {
									echo ($c != 0 ? ', ' : '').html::anchor('status/'.$grouptype.'group/'.urlencode($details->groupname).'?style=detail&servicestatustypes='.nagstat::SERVICE_WARNING.'&hoststatustypes='.(nagstat::HOST_DOWN|nagstat::HOST_UNREACHABLE), $details->services_warning_host_problem.' '.$label_on_problem_hosts);
									$c++;
								}
								if ($details->services_warning_scheduled > 0) {
									echo ($c != 0 ? ', ' : '').html::anchor('status/'.$grouptype.'group/'.urlencode($details->groupname).'?style=detail&servicestatustypes='.nagstat::SERVICE_WARNING.'&serviceprops='.nagstat::SERVICE_SCHEDULED_DOWNTIME, $details->services_warning_scheduled.' '.$label_scheduled);
									$c++;
								}
								if ($details->services_warning_acknowledged > 0) {
									echo ($c != 0 ? ', ' : '').html::anchor('status/'.$grouptype.'group/'.urlencode($details->groupname).'?style=detail&servicestatustypes='.nagstat::SERVICE_WARNING.'&serviceprops='.nagstat::SERVICE_STATE_ACKNOWLEDGED, $details->services_warning_acknowledged.' '.$label_acknowledged);
									$c++;
								}
								if ($details->services_warning_disabled > 0) {
									echo ($c != 0 ? ', ' : '').html::anchor('status/'.$grouptype.'group/'.urlencode($details->groupname).'?style=detail&servicestatustypes='.nagstat::SERVICE_WARNING.'&serviceprops='.nagstat::SERVICE_CHECKS_DISABLED, $details->services_warning_disabled.' '.$label_disabled);
									$c++;
								}
								echo '<br />';
							}
							...
							if ($details->services_unknown > 0) {
								...
								if ($details->services_unknown_unhandled > 0) {
									echo ($c != 0 ? ', ' : '').html::anchor('status/'.$grouptype.'group/'.urlencode($details->groupname).'?style=detail&servicestatustypes='.nagstat::SERVICE_UNKNOWN.'&hoststatustypes='.(nagstat::HOST_UP|nagstat::HOST_PENDING).'&serviceprops='.(nagstat::SERVICE_NO_SCHEDULED_DOWNTIME|nagstat::SERVICE_STATE_UNACKNOWLEDGED|nagstat::SERVICE_CHECKS_ENABLED), $details->services_unknown_unhandled.' '.$label_unhandled);
									$c++;
								}
								if ($details->services_unknown_host_problem > 0) {
									echo ($c != 0 ? ', ' : '').html::anchor('status/'.$grouptype.'group/'.urlencode($details->groupname).'?style=detail&servicestatustypes='.nagstat::SERVICE_UNKNOWN.'&hoststatustypes='.(nagstat::HOST_DOWN|nagstat::HOST_UNREACHABLE), $details->services_unknown_host_problem.' '.$label_on_problem_hosts);
									$c++;
								}
								if ($details->services_unknown_scheduled > 0) {
									echo ($c != 0 ? ', ' : '').html::anchor('status/'.$grouptype.'group/'.urlencode($details->groupname).'?style=detail&servicestatustypes='.nagstat::SERVICE_UNKNOWN.'&serviceprops='.nagstat::SERVICE_SCHEDULED_DOWNTIME, $details->services_unknown_scheduled.' '.$label_scheduled);
									$c++;
								}
								if ($details->services_unknown_acknowledged > 0) {
									echo ($c != 0 ? ', ' : '').html::anchor('status/'.$grouptype.'group/'.urlencode($details->groupname).'?style=detail&servicestatustypes='.nagstat::SERVICE_UNKNOWN.'&serviceprops='.nagstat::SERVICE_STATE_ACKNOWLEDGED, $details->services_unknown_acknowledged.' '.$label_acknowledged);
									$c++;
								}
								if ($details->services_unknown_disabled > 0) {
									echo ($c != 0 ? ', ' : '').html::anchor('status/'.$grouptype.'group/'.urlencode($details->groupname).'?style=detail&servicestatustypes='.nagstat::SERVICE_UNKNOWN.'&serviceprops='.nagstat::SERVICE_CHECKS_DISABLED, $details->services_unknown_disabled.' '.$label_disabled);
									$c++;
								}
								echo '<br />';
							}
							...
							if ($details->services_critical > 0) {
								...
								if ($details->services_critical_unhandled > 0) {
									echo ($c != 0 ? ', ' : '').html::anchor('status/'.$grouptype.'group/'.urlencode($details->groupname).'?style=detail&servicestatustypes='.nagstat::SERVICE_CRITICAL.'&hoststatustypes='.(nagstat::HOST_UP|nagstat::HOST_PENDING).'&serviceprops='.(nagstat::SERVICE_NO_SCHEDULED_DOWNTIME|nagstat::SERVICE_STATE_UNACKNOWLEDGED|nagstat::SERVICE_CHECKS_ENABLED), $details->services_critical_unhandled.' '.$label_unhandled);
									$c++;
								}
								if ($details->services_critical_host_problem > 0) {
									echo ($c != 0 ? ', ' : '').html::anchor('status/'.$grouptype.'group/'.urlencode($details->groupname).'?style=detail&servicestatustypes='.nagstat::SERVICE_CRITICAL.'&hoststatustypes='.(nagstat::HOST_DOWN|nagstat::HOST_UNREACHABLE), $details->services_critical_host_problem.' '.$label_on_problem_hosts);
									$c++;
								}
								if ($details->services_critical_scheduled > 0) {
									echo ($c != 0 ? ', ' : '').html::anchor('status/'.$grouptype.'group/'.urlencode($details->groupname).'?style=detail&servicestatustypes='.nagstat::SERVICE_CRITICAL.'&serviceprops='.nagstat::SERVICE_SCHEDULED_DOWNTIME, $details->services_critical_scheduled.' '.$label_scheduled);
									$c++;
								}
								if ($details->services_critical_acknowledged > 0) {
									echo ($c != 0 ? ', ' : '').html::anchor('status/'.$grouptype.'group/'.urlencode($details->groupname).'?style=detail&servicestatustypes='.nagstat::SERVICE_CRITICAL.'&serviceprops='.nagstat::SERVICE_STATE_ACKNOWLEDGED, $details->services_critical_acknowledged.' '.$label_acknowledged);
									$c++;
								}
								if ($details->services_critical_disabled > 0) {
									echo ($c != 0 ? ', ' : '').html::anchor('status/'.$grouptype.'group/'.urlencode($details->groupname).'?style=detail&servicestatustypes='.nagstat::SERVICE_CRITICAL.'&serviceprops='.nagstat::SERVICE_CHECKS_DISABLED, $details->services_critical_disabled.' '.$label_disabled);
									$c++;
								}
								echo '<br />';
							}
