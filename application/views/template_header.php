...
?><div class="header_info_row"><?php
	...
	<form action="<?php echo Kohana::config('config.site_domain') ?><?php echo Kohana::config('config.index_page') ?>/search/lookup" method="get">
		<?php echo _('Welcome'); ?> <?php echo html::anchor('user', strlen(user::session('realname')) > 0 ? htmlentities(user::session('realname')) : user::session('username')) ?> | <?php echo html::anchor('default/logout', html::specialchars(_('Log out'))) ?><br />
		<?php
