...
module Configuration
	...
	class NagiosConfiguration
		...
		def initialize(template_dir="test/configs/templates/etc")
			...
			@root_path = Dir.mktmpdir("config_")
			@objects = {}
		end
		...
		def activate
			...
			patch_nacoma_config
			patch_op5lib_config
			spawn_child "%s -d %s" % [@nagios_bin, nagios_cfg]
			begin
				file_wait(@cmd_file, :state => :exists)
			rescue Exception => e
				nagios_log = File.join(@var_path, "nagios.log")
				puts "==== NAGIOS LOG ===="
				puts `tail #{nagios_log}`
				puts "==== NAGIOS LOG END ===="
				raise e
			end
			file_wait(File.join(@rw_path, "live"))
		...
		##
		# @param ls_sock_path [String]
		# @raise [RuntimeError] Livestatus was not enabled
		def check_livestatus(ls_sock_path)
			IO.popen("python2.6 " + File.dirname(__FILE__) + "/scripts/check_livestatus.py " + ls_sock_path) { |p|
				livestatus_check_output = p.read

			}
			if ($?.exitstatus != 0)
				puts livestatus_check_output
				raise "Livestatus not up :("
			end
		end
		...
		def verify_configuration(cfg)
			nagios_cfg = File.join(@etc_path, "etc", "nagios.cfg")
			IO.popen(@nagios_bin + " -v " + cfg) {|p|
		...
		def set_nagios_pipe(pipe_path)
			...
			if not File.exist? config_php
				...
				puts "Cowardly refusing to overwrite existing custom config at %s, might not be a problem unless these are Ninja tests" % config_php
			end
		end
